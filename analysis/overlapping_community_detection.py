...
# number_communities = 5  # 可根据需要调整
# conga_communities = algorithms.conga(G, number_communities=number_communities)

# # 输出整体扩展模块度（EQ）
# eq_conga = evaluation.erdos_renyi_modularity(G, conga_communities).score
# print(f"EQ (CONGA): {eq_conga:.4f}")

# # 统计社区数和每个社区的节点数（已按规模从大到小排序）
# conga_community_list = sorted(conga_communities.communities, key=len, reverse=True)
# conga_community_sizes = [len(c) for c in conga_community_list]
# conga_community_densities = [nx.density(G.subgraph(c)) for c in conga_community_list]
# print(f"Number of communities (CONGA): {len(conga_community_list)}")
# for i, (c, d) in enumerate(zip(conga_community_list, conga_community_densities)):
#     print(f"Community {i+1}: {len(c)} nodes, density={d:.4f}")

# # 使用 cdlib.viz.plot_network_highlighted_clusters 绘制社区高亮图
# viz.plot_network_highlighted_clusters(G, conga_communities, position=position, figsize=(12, 9), node_size=30)
# plt.title(f'Overlapping Community Visualization (CONGA, highlighted clusters, k={number_communities})')
# plt.tight_layout()
# plt.savefig('./data/overlapping_community_colored_conga.png')
# plt.show()

# fig, ax1 = plt.subplots(figsize=(12,6))
# x = np.arange(1, len(conga_community_sizes)+1)
# color1 = 'tab:blue'
# color2 = 'tab:orange'
# ax1.set_xlabel('Community Index (sorted by size)')
# ax1.set_ylabel('Community Size', color=color1)
# ln1 = ax1.plot(x, conga_community_sizes, color=color1, marker='o', label='Community Size')
# ax1.tick_params(axis='y', labelcolor=color1)
# ax1.set_xticks(x)

# ax2 = ax1.twinx()
# ax2.set_ylabel('Density', color=color2)
# ln2 = ax2.plot(x, conga_community_densities, color=color2, marker='s', label='Community Density')
# ax2.tick_params(axis='y', labelcolor=color2)
# ax2.set_xticks(x)
